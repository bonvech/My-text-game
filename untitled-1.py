...
while truea != 1: # соблюдает ли слово нужные нам условия
    a = input('Здраствуйте, сегодня сыграем виселицу загадайте русское слово:') #проверка на правильность ввода
    a = a.lower()
    for i in a:
...
while count != 0:
    ...
    if (b == a) and (len(b) > 1):# проверка на правильность всей строки
        print("Ты угадал!!!")
        print()
        print(count, 'это твоё количество попыток на неправильный ввод')
        ...
        for i in a:
            ...
            if b == i:
                for i in star:
                    if i == '*':
                        star[counta] =  b 
        print(count, 'количество оставшихся попыток')
        for i in star:
            print(i, end='')      
        massiveest.add(b)
        ...
        print(count, "сколько осталось попыток")
    elif b == '!comands':
        ...
        print('!slovo мини режим, в котором у тебя будет одна попытка на то, что бы угадать слово, но все буквы будут даны 1 раз')
